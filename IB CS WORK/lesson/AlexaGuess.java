 ...
 * Filling in the comments is not optional so do not ask whether you can not do
 * it *eye roll*.
 */
public class AlexaGuess {
    public static void main(String[] args) {
        ...
        if (response.equals("yes") || response.equals("y")) {
            ...
            // literally just a ... to convey the image of Alexa thinking (even though it
            // took her less than a second)
            System.out.print(".");
            pause(1);
            System.out.print(".");
            pause(1);
            ...
            while (guess != number && triesLeft > 0) {
                if (guess < number) { // If Users number guessed is smaller, alexa gives a hint that it is larger
                    System.out.println(
                            "You are not correct! The number I am thinking of, is larger. Take another guess...");
                } else { // If Users number guessed is larger, alexa gives a hint that it is smaller
                    System.out.println(
                            "You are not correct! The number I am thinking of, is smaller. Take another guess...");
                }
            ...
            if (guess == number) { // If user guessed the number in under 3/4 tries, prints response
                System.out.println("Well done, you guessed. That was a good round.");
            } else { // If user did not guess the number correctly in under 3/4 tries, prints
                     // response and tells answer
